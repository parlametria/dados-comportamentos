...
export_filepath: Caminho para exportação dos dados de tweets que falam sobre proposições
"
...
get_args <- function() {
  ...
  option_list = list(
    ...
    optparse::make_option(c("-e", "--export_filepath"),
                          type="character",
                          default=here::here("data/ready/tweets_proposicoes/tweets_proposicoes_socioambiental.csv"),
                          help=.HELP,
                          metavar="character")
  );
...
tweets_proposicoes <- fetch_tweets(url, painel, data_inicial, data_final)
write_csv(tweets_proposicoes, here::here(saida))
