...
padroniza_cargo_comissao <- function(cargo) {
  library(tidyverse)
  source(here::here("code/comissoes/constants.R"))
...
#' @param data_inicio Data de início de interesse no formato AAAA-MM-DD.
#' @return Lista com dois Dataframes: comissões e composição das comissões
#' @examples
#' processa_comissoes()
processa_comissoes <- function(data_inicio = "2019-02-01") {
  library(tidyverse)
  library(here)
  source(here("code/comissoes/fetcher_comissoes.R"))
  
  comissoes <- fetch_comissoes_composicao()
  ...
  lista_comissao <- comissao_composicao %>% 
    ...
    dplyr::mutate(dados = purrr::map2(sigla, 
                                      casa,
                                      fetch_comissao_info)) %>% 
    tidyr::unnest(dados, names_repair="universal") %>% 
  ...
  composicao_comissoes <- comissao_composicao %>% 
    ...
    mutate(id_parlamentar_parlametria = paste0(casa_enum, id)) %>% 
    dplyr::select(comissao_id, sigla, casa, id_parlamentar_parlametria, cargo, situacao, data_inicio, data_fim, is_membro_atual)
