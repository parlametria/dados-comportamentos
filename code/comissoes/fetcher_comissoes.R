...
#' @examples
#' fetch_comissoes_composicao()
fetch_comissoes_composicao <- function(data_inicio = '2019-02-01') {
  return(agoradigital::fetch_all_composicao_comissao(data_inicio))
}
...
fetch_comissao_info_senado <- function(sigla_comissao) {
  library(tidyverse)
  
  url <- paste0("https://legis.senado.leg.br/dadosabertos/comissao/lista/permanente")
  
  comissao <- tryCatch({
    xml <- RCurl::getURL(url) %>% xml2::read_xml()
    data <- xml2::xml_find_all(xml, ".//colegiado") %>%
      map_df(function(x) {
        list(
          comissao_id = xml2::xml_find_first(x, ".//CodigoColegiado") %>% 
            xml2::xml_text(),
          sigla = xml2::xml_find_first(x, ".//SiglaColegiado") %>% 
            xml2::xml_text(),
          nome_comissao = xml2::xml_find_first(x, ".//NomeColegiado") %>% 
            xml2::xml_text()
        )
      }) %>% 
      distinct() %>% 
      filter(sigla == sigla_comissao) %>% 
