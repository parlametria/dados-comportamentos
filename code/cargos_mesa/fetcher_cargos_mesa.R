...
fetch_cargos_mesa_camara <-
  function(legislatura = 56,
           atual_cargo = TRUE) {
    library(tidyverse)
    ...
    if (atual_cargo) {
      legislatura = 56
      message(
        "Aviso: parâmetro atual_cargo tem valor TRUE. Valor da legislatura utilizado será 56 (atual)."
      )
    } else {
      message(paste0("Baixando os cargos de mesa para a legislatura ", legislatura))
    }
    ...
    if (length(dados_raw) == 0) {
      message("Não há dados de cargos na mesa para esta legislatura.")
      return(tibble())
    }
...
#' @return Dataframe contendo deputados com cargos na Mesa da Câmara.
#' @examples
#' deputados_mesa <- fetch_cargos_mesa_camara_all_legislaturas()
fetch_cargos_mesa_camara_all_legislaturas <-
  function(legislaturas = seq(52, 56)) {
    cargos_mesa <-
      purrr::map_df(legislaturas, ~ fetch_cargos_mesa_camara(.x, atual_cargo = F))
...
fetch_cargos_mesa_senado <- function() {
  ...
  url <-
    "https://legis.senado.leg.br/dadosabertos/dados/MesaSenado.xml"
...
check_cargos <- function(df) {
  ...
  source(here("code/cargos_mesa/constants.R"))
  ...
  lista_cargos <-
    c(
      ...
      .VICE_PRESIDENTE_2,
      .PRESIDENTE,
      .SUPLENTE_SECRETARIO_1,
      .SUPLENTE_SECRETARIO_2,
      .SUPLENTE_SECRETARIO_3,
      .SUPLENTE_SECRETARIO_4,
      .SUPLENTE_1,
      .SUPLENTE_2,
      .SUPLENTE_3,
      .SUPLENTE_4
